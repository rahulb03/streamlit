...
def get_navamsha_sign(longitude):
    """Calculates the D9 (Navamsha) sign for a given longitude."""
    sign_index = int(longitude // 30)
    degree_in_sign = longitude % 30
    navamsha_part = int(degree_in_sign // (30 / 9))
    
    # 1. Fire Signs (Aries 0, Leo 4, Sag 8) -> Start from Aries (0)
    # 2. Earth Signs (Taurus 1, Virgo 5, Cap 9) -> Start from Capricorn (9)
    # 3. Air Signs (Gemini 2, Libra 6, Aqua 10) -> Start from Libra (6)
    # 4. Water Signs (Cancer 3, Scorpio 7, Pisces 11) -> Start from Cancer (3)
    
    if sign_index in [0, 4, 8]: start_index = 0
    elif sign_index in [1, 5, 9]: start_index = 9
...
def get_timezone_offset(lat, lon, date_obj, time_obj):
    tf = TimezoneFinder()
    timezone_name = tf.timezone_at(lat=lat, lng=lon)
    if not timezone_name: return None
    local_tz = pytz.timezone(timezone_name)
    naive_dt = datetime.combine(date_obj, time_obj)
    local_dt = local_tz.localize(naive_dt)
    return (local_dt.utcoffset().total_seconds()) / 3600.0
...
def calculate_chart(name, dob, tob, lat, lon, tz_offset):
    ...
    # 1. Calculate Sun first (needed for combustion)
    xx, _ = swe.calc_ut(jd_ut, swe.SUN)
    sun_longitude = (xx[0] - ayanamsa) % 360
    
    # Helper to process planet data
    def process_planet(name, id, p_type):
        xx, _ = swe.calc_ut(jd_ut, id)
        sid_lon = (xx[0] - ayanamsa) % 360
        is_retro = xx[3] < 0
        house = find_house(sid_lon, sidereal_cusps)
        sign = get_sign(sid_lon)
        
        # Status Checks
        is_combust = False
        if name != "Sun" and name in COMBUSTION_ORBS:
            diff = abs(sid_lon - sun_longitude)
            if diff > 180: diff = 360 - diff
            if diff < COMBUSTION_ORBS[name]:
                is_combust = True
                
        is_exalted = (sign == EXALTATION_SIGNS.get(name))
        is_debilitated = (sign == DEBILITATION_SIGNS.get(name))
        
        # Vargottama Check (Same sign in Rasi and Navamsha)
        navamsha_sign = get_navamsha_sign(sid_lon)
        is_vargottama = (sign == navamsha_sign)

        return {
            "degree": sid_lon, "sign": sign, "house": house, 
            "is_retrograde": is_retro, "type": p_type,
            "status": {
                "combust": is_combust,
                "exalted": is_exalted,
    ...
    for p_name, p_id in OUTER_PLANET_IDS.items():
        ...
        house = find_house(sid_lon, sidereal_cusps)
        planets[p_name] = {
            "degree": sid_lon, "sign": get_sign(sid_lon), "house": house, 
            "is_retrograde": xx[3] < 0, "type": "outer",
            "status": {"combust": False, "exalted": False, "debilitated": False, "vargottama": False}
        }
    ...
    for name, lon in [("Rahu", rahu_lon), ("Ketu", ketu_lon)]:
        sign = get_sign(lon)
        navamsha_sign = get_navamsha_sign(lon)
        planets[name] = {
            "degree": lon, "sign": sign, "house": find_house(lon, sidereal_cusps), 
            "is_retrograde": True, "type": "traditional",
            "status": {
                "combust": False, 
                "exalted": (sign == EXALTATION_SIGNS.get(name)),
                "debilitated": (sign == DEBILITATION_SIGNS.get(name)),
                "vargottama": (sign == navamsha_sign)
            }
        }
...
def generate_north_indian_svg(chart):
    ...
    for p_name, data in planets.items():
        ...
        house_content[h_num].append(f"{PLANET_ABBR.get(p_name, p_name)} {deg}° {indicator_str}")
    ...
    for h in range(1, 13):
        ...
        start_y = y + 5
        for i, p_str in enumerate(planets_in_house):
            if "Asc" in p_str: color = "#AA0000"; weight = "bold"
            elif any(x in p_str for x in ["Su", "Ma"]): color = "#D32F2F"; weight = "normal"
            elif any(x in p_str for x in ["Ur", "Ne", "Pl"]): color = "#555555"; weight = "normal"
            else: color = "#1976D2"; weight = "normal"
                
            svg += f'<text x="{x}" y="{start_y + (i*14)}" font-family="Arial" font-size="10" font-weight="{weight}" fill="{color}" text-anchor="middle">{p_str}</text>'
...
class MockLalKitabPredictor:
    ...
    def generate_chart_predictions(self, chart):
        ...
        for p, data in chart['planets'].items():
            preds[p] = {
                "house": data['house'], "sign": data['sign'],
                "prediction": f"General prediction for {p}.", "remedy": f"Remedy for {p}."
            }
        return preds
            ...
            if tz_offset is None:
                ...
                with tab2:
                    ...
                    with c1:
                        ...
                        svg_chart = generate_north_indian_svg(full_chart)
                        st.markdown(svg_chart, unsafe_allow_html=True)
                        st.caption("Symbols: (R)=Retrograde, (C)=Combust, (V)=Vargottama, ↑=Exalted, ↓=Debilitated")
                    ...
                    with c2:
                        ...
                        planet_display_list.append({
                            "Planet": "Ascendant", "Sign": full_chart['ascendant']['sign'],
                            "Degree": f"{asc_d}° {asc_m}'", "House": 1, "Status": "-"
                        })
                        ...
                        for p, d in full_chart['planets'].items():
                            ...
                            status_list = []
                            if d.get('is_retrograde'): status_list.append("Retrograde")
                            s = d.get('status', {})
                            if s.get('combust'): status_list.append("Combust")
                            if s.get('vargottama'): status_list.append("Vargottama")
                            if s.get('exalted'): status_list.append("Exalted")
                            if s.get('debilitated'): status_list.append("Debilitated")
